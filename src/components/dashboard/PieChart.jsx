...
export function PieChart() {
  ...
  return (
    <div className="pie-chart-box space-y-6 flex flex-col w-full px-6 py-0 pt-4 sm:px-10 sm:py-7">
      {/* Heading */}
      ...
      <div className="flex flex-row justify-between">
        ...
        <div className="flex flex-col items-start space-y-2 ">
          {Object.entries(pieChartData).map(([key, value]) => {
            return (
              <div className="flex items-start space-x-2 flex-row" key={key}>
                <div
                  className={`rounded-full mt-1 w-4 h-4 ${getClassBgOfPie(
                    key
                  )} `}
                ></div>
