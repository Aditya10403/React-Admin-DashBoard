    ...
    setEvents((prevEvents) => {
      return prevEvents.map((e) => {
        return e.id === event.id
          ? { ...event, data: { ...event.data, title, label } }
          : e;
      });
    });
        ...
        <div className="box relative  bg-white rounded-md  shadow-2xl h-96 w-[24rem] px-5 py-5">
                ...
                {["red", "green", "blue", "yellow", "purple"].map((lbl) => {
                  return (
                    <div
                      ...
                      className={`${
                        label === lbl ? `bg-${lbl}-800` : ""
                      } cursor-pointer bg-${lbl}-500 px-3 py-3 rounded-full relative`}
                    >
                      <div
                        className={`${
                          label === lbl ? "" : "hidden"
                        } tick text-white absolute top-1 right-1`}
                      >
                        <MdOutlineDone />
                      </div>
                    </div>
          ...
          <div className="absolute bottom-3 right-3">
            <button
              onClick={deleteEvent}
              type="button"
              class="text-gray-900 bg-red border border-red-300 focus:outline-none hover:bg-red-100 focus:ring-4 focus:ring-red-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb- "
            >
              Delete
            </button>
